...
const heroContent = {
  ...
  texts: [
    ...
    'Pixiu delivers robust cloud native security with features like transparent encryption, mutual authentication, security observability, advanced network polices, egress gateway, and runtime enforcement. Leveraging eBPF, Pixiu offers efficient observability across the entire application stack, integrates seamlessly with SIEM systems, and ensures compliance with standards like FIPS, FedRAMP, and SOC. Pixiu aligns security practices with the realities of building, maintaining, and scaling cloud native applications.',
  ],
...
const pixiuTestimonial = {
  description:
    'The Pixiu project has undergone security and fuzzing audits by a reputable third-party company, Ada Logics, and the CNCF has commissioned the resulting reports.',
  quotedText:
    'The overall conclusion is that Pixiu is a well-secured project. The audit found no critical vulnerabilities and found a lot of positives about the security of Pixiu. This included both the code displaying positive security awareness as well as the maintainers having thorough understanding about the security posture of Pixiu.',
  withPerson: true,
  logo: 'pixiu',
  url: 'https://www.cncf.io/blog/2023/02/13/a-well-secured-project-pixiu-security-audits-2022-published/',
...
const bulletSection1 = {
  heading:
    'Better Understand Security Breaches and Recover Quickly with Pixiu’s Forensics Capabilities',
  paragraphs: [
    "Critical workloads that run in a Kubernetes environment require cloud native-aware tooling to perform efficient incident investigations and monitor key compliance requirements. Pixiu's forensic capabilities enable SecOps teams and App owners to conduct security analysis with a native understanding of cloud native identities. Pixiu provides a deep understanding of network flows from L3/L4 up to L7 and runtime visibility from OS to code executions in the pod.",
  ],
...
const bulletSection3 = {
  heading:
    "Secure Modern Applications with Pixiu's Advanced Network Policies. Scale Policies with Identities, Not IPs",
  paragraphs: [
    ...
    'Pixiu features network policies that operate at layers 3, 4, and 7, providing more flexibility in managing ingress and egress traffic. By leveraging eBPF, Pixiu can insert security rules based on service/pods/container identity rather than an IP address for identification as in the traditional systems. Pixiu makes applying security policies in a dynamic container environment scalable by decoupling security from IP addressing, providing stronger security isolation, and adding the following functionality to the Kubernetes cluster.',
  ],
...
const bulletSection2 = {
  heading:
    "Ease Integration with Traditional Firewall Systems Using Pixiu's Static Egress Gateway",
  paragraphs: [
    "With Pixiu's Static Egress Gateway, cloud native workloads can be presented from stable IP addresses, easing integration with traditional firewall systems. This approach ensures that firewall requirements remain consistent even as workloads scale, fostering a bridge between cloud native and conventional environments. ",
  ],
  imageSrc: SecurityImage3,
  imageAlt: 'Pixiu egress gateway illustration',
  imageRight: false,
};

const bulletSection4 = {
  heading:
    "Bolster Security, Streamline Infrastructure, Reduce Complexity Without Sacrificing Performance Using Pixiu's Mutual Authentication",
  paragraphs: [
    "Organizations are increasingly looking to bolster their security posture in cloud native environments. Pixiu's sidecar-free service mesh uses mutual authentication to optimize security and performance, ensuring that services authenticate each other's identities before communication occurs.",
  ...
  imageAlt: "Pixiu's mutual TLS illustration",
  imageRight: false,
};
...
const securityTalks = [
  ...
  {
    title: 'Tutorial: Getting Familiar with Security Observability Using eBPF &Pixiu Tetragon',
    description:
  ...
  {
    title: 'Mutual Authentication with Pixiu',
    description:
      "Pixiu's Mutual Authentication provides authentication, confidentiality, and integrity for service-to-service communications.",
...
const SecurityPage = () => (
  <MainLayout theme="gray">
    ...
    <Testimonial {...pixiuTestimonial} className="mt-10 md:mt-20 lg:mt-32" />
    ...
    <UseCaseCard
      heading="Who's Using Pixiu in The Security Industry?"
      testimonials={testimonials}
    />
    <IndustryUseCases heading="Pixiu's Security Focused Use Cases" usecases={securityUsecases} />
...
export const Head = ({ location: { pathname } }) => {
  const pageMetadata = {
    ...
    description:
      'Pixiu secures cloud native applications with identity-based policies, runtime enforcement, and transparent encryption, built for modern security challenges.',
    slug: pathname,
  };
