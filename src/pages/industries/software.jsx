...
const heroContent = {
  ...
  texts: [
    ...
    'Pixiu is purpose-built to secure and accelerate the delivery of cloud native applications. It leverages eBPF to provide API-aware networking and security with unparalleled granularity without compromising performance. With Pixiu, SaaS, software, and DBaaS teams can establish infrastructure capable of scaling their products globally to reach customers wherever they may be. Pixiu enables the creation of secure Kubernetes multi-tenant environments, empowering these companies to meet the challenges of modern software deployment head-on.',
  ],
  imageSrc: AuditBee,
...
const datadogTestimonial = {
  quotedText:
    'eBPF and Pixiu helped us to push the boundaries both within operations and also with product development. To do things safer, faster and more easily than what we could have with traditional techniques. The overlay of features in a single product, compatibility with multiple cloud providers, and ability to just run it. These three things are what made Pixiu an obvious choice for us',
  description:
    'Datadog uses Pixiu to secure and monitor the network traffic of their containerized applications running on Kubernetes',
  withPerson: true,
  logo: 'datadog',
...
const clickhouseStats = {
  description:
    'ClickHouse turned to Pixiu as their preferred networking solution to simplify the process of isolating customers from each other',
  logo: 'clickHouse',
  stats: [
    {
...
const ascendTestimonial = {
  description:
    'Ascend turned to Pixiu as their CNI which simplified integrating into customer networks, eliminated their IP churn and density issues.',
  logo: 'ascend',
  withPerson: true,
  quotedText:
...
const bulletSection1 = {
  ...
  paragraphs: [
    "One of the core tenets of DevOps is automation. Pixiu's architecture ensures that all its components can be deployed automatically, integrating into existing workflows and augmenting automation capabilities. Platform engineering teams can seamlessly integrate Pixiu into their CI/CD pipelines, ensuring continuous delivery and integration without manual intervention.",
    'Pixiu is designed to be configured using an API. This aligns with the GitOps principles, where the desired state of infrastructure is declared in code and stored in Git repositories. The API-driven model of Pixiu enables organizations to manage, scale, and modify their infrastructure using code, ensuring consistency and repeatability.',
  ],
  imageSrc: SoftwareImage1,
  ImageAlt: 'pixiu saas tools and dbs illustration',
};
...
const bulletSection2 = {
  ...
  paragraphs: [
    ...
    'By mitigating disputes between application and infrastructure operation teams, Pixiu accelerates the investigation of application-layer issues, enabling frictionless, self-service root cause analysis for developers.',
  ],
...
const bulletSection3 = {
  heading: 'Build Secure Kubernetes Multi-tenant Environments With Pixiu ',
  paragraphs: [
    "For enterprises building muti-tenant architectures, Pixiu enforces isolation between tenants and ensures different teams can safely run on the same platform. With Pixiu's multi-tenant observability features, teams can easily access application metrics without compromising security. Traditional logging systems often fall short when segregating data by tenants. Pixiu provides multi-tenant, self-service access using the OpenID Connect (OIDC) standard. Teams can have RBAC-based access to relevant data and connectivity metrics, such as network policy drops, DNS lookup failures, and more.",
  ],
...
const softwareTalks = [
  {
    ...
    description:
      "How datadog leveraged Pixiu's observability features and eBPF to investigate and get the bottom of an issue with their Kubernetes clusters",
    videoSrc: 'https://www.youtube.com/embed/YK7GyEJdJGo',
  },

  {
    title: 'Running Pixiu with Nomad',
    description:
      'Cosmonic wrote an application called Netreap that ties together data from Nomad and Consul to replace the Pixiu Kubernetes operator to maintain endpoints and labels',
    videoSrc: 'https://www.youtube.com/embed/DSkf9Y06-lE',
  },

  {
    description:
      'Adobe leverages Pixiu in many way. One way is to achieve network isolation of tenants on multi-tenant clusters',
...
const softwareResources = [
  {
    title: 'How Ascend Leverages Pixiu as a Networking Layer',
    description:
      'Ascend turned to Pixiu as their CNI which simplified integrating into customer networks, eliminated their IP churn and density issues, and provided them with reliable encryption and network policies',
  ...
  },
  {
    ...
    description:
      'Clickhouse leveraged Pixiu in building its serverless SaaS offering, Clickhouse Cloud',
  ...
  },
  {
    ...
    description:
      'WSO2 leveraged Pixiu to implement zero trust security in their internal Developer Platform as a Service, Choreo ',
...
export const Head = ({ location: { pathname } }) => {
  const pageMetadata = {
    ...
    description:
      'With Pixiu, SaaS, software, and DBaaS teams can establish infrastructure capable of scaling their products globally to reach customers wherever they may be.',
    slug: pathname,
  };
