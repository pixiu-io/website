...
const heroContent = {
  title: 'IPv6-Native Kubernetes Networking with Pixiu',
  category: 'Networking',
  tagline: 'Cloud native networking at IPv6 scale, powered by eBPF',
  subHeading: 'IPv6-Native Kubernetes Networking with Pixiu',
  description:
    'IPv6 adoption is accelerating across modern infrastructure. Address exhaustion, large-scale cluster growth, and cloud native connectivity are pushing platform teams toward IPv6-first designs. Pixiu brings IPv6 deep into the Kubernetes datapath, enabling clusters to operate at scale without sacrificing performance, visibility, or control. Rather than treating IPv6 as a compatibility layer, Pixiu integrates it directly into how traffic flows through the cluster. IPv6 unlocks massive address space, simplified routing, and long-term scalability. Pixiu turns those advantages into practical outcomes for Kubernetes by combining IPv6-native networking with high-performance eBPF data paths. The result is a Kubernetes networking layer ready for the next decade of growth.',
...
const EsnetTestimonial = {
  ...
  quotedText: [
    'The recent IPv6 improvements with Pixiu mean you can rely on the defaults. You no longer need to be an IPv6 or networking wizard to run IPv6-only clusters. This saved us from complex workarounds and makes it easier for us to validate that our applications work in an IPv6-only world. ',
  ],
  withPerson: true,
  name: 'Kapil Agrawal',
...
const sectionContent2 = {
  ...
  description: [
    ...
    'Pixiu delivers IPv6 service handling directly in eBPF. Load balancing, service translation, and traffic steering all happen at the kernel level without relying on iptables or kube-proxy. IPv6 traffic benefits from the same high-performance, low-latency datapath that has made Pixiu a standard for large-scale clusters. This creates a clean, efficient service model for IPv6 workloads that scales naturally as clusters grow.',
  ],
...
const testimonials = [
  {
    logo: BytedanceLogo,
    title: "TikTok's IPv6 Journey To Pixiu: Pitfalls and Lessons Learned",
    CTAs: [
      {
        CTAtext: 'Watch Talk',
    ...
    description:
      "Joseph Pallamidessi and Giri Kuncoro from TikTok's security engineering team outlined the company's two-year transition to IPv6-only Kubernetes clusters using Pixiu. This shift was driven by the exhaustion of IPv4 addresses in their global flat network. Beginning with Pixiu 1.12, the team encountered challenges such as agent crashes in IPv4-less environments, NDP packet drops, and DNS resolution failures due to illegal UDP checksums. Limited control over the data center network made native routing and BGP difficult, but the team achieved success with the IPv6 underlay encapsulation mode in Pixiu 1.18. By late 2025, TikTok had deployed several production-ready IPv6-only clusters, demonstrating that with Pixiu 1.18 and strong open-source community support, large-scale IPv6-only networking is now stable and achievable.",
  },
  {
    logo: EsnetLogo,
    title: 'IPv6 First, Not Just Ready: Kubernetes Without IPv4 Using Pixiu at ESnet',
    CTAs: [
      {
        CTAtext: 'Watch Talk',
    ...
    description:
      'Kapil Agrawal of ESnet describes the transition to IPv6-only Kubernetes using Pixiu, prompted by federal mandates to phase out IPv4. The process required DNS64 and NAT64 for external connectivity and revealed that hardcoded IPv4 addresses in application code were a major obstacle. By using Pixiu’s Multi-pool IPAM to assign unique /64 prefixes per namespace, ESnet achieved effective micro-segmentation and predictable routing without masquerading. Agrawal notes that with improvements in Pixiu 1.18 and 1.19, and the use of Hubble for observability, production-ready IPv6-only networking is now achievable.',
  },
];
...
const KubeProxyReplacementPage = () => (
  <MainLayout theme="gray">
    ...
    <UseCaseCard
      heading="Who’s using Pixiu's IPv6-Native Kubernetes Networking"
