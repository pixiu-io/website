/* eslint-disable react/prop-types */
import React from 'react';

import Logos from 'components/pages/adopters/logos';
import HeroWithoutImage from 'components/shared/hero-without-image';
import SEO from 'components/shared/seo';
import UserCommunity from 'components/shared/user-community';
import MainLayout from 'layouts/main/main';
import caseStudiesData from 'utils/case-studies-data';
import { adopters as seo } from 'utils/seo-metadata';

const {
  FHNW,
  koyeb,
  bmc,
  labyrinthLabs,
  scigility,
  syseleven,
  icos,
  nutanix,
  cloudera,
  uwmadison,
  cloutomate,
  qpoint,
  seatgeek,
  archer,
  kakao,
  samsungads,
  VMwareTanzu,
  bitnami,
  nemlig,
  linkpool,
  aenix,
  syself,
  spectrocloud,
  gdata,
  smartnews,
  roche,
  sicredi,
  gcore,
  netbird,
  veepee,
  rabobank,
  netways,
  sealos,
  santa,
  jsyk,
  vkcloud,
  sianalytics,
  ilionx,
  cybozu,
  viasat,
  ws02,
  singlestore,
  deezer,
  parseable,
  plusserver,
  dbraincloud,
  dbschenker,
  twilio,
  testifysec,
  trendyol,
  eficode,
  tietoevry,
  unitedcloud,
  planetscale,
  nine,
  kubehetzner,
  kilo,
  polverio,
  polarsignals,
  daimlertruck,
  clickhouse,
  proton,
  cistec,
  bloomberg,
  azure,
  robinhood,
  vshn,
  edgelessSystems,
  giantSwarm,
  immerok,
  magicLeap,
  sapian,
  ungleich,
  elasticpath,
  bytedance,
  seznam,
  sandp,
  google,
  sproutfi,
  superorbital,
  acoss,
  arangodb,
  bell,
  gitlab,
  aws,
  wildLife,
  adobe,
  accuknox,
  capitalOne,
  alibabaCloud,
  masmovil,
  trip,
  cengn,
  digitalOcean,
  canonical,
  civo,
  sky,
  solo,
  uswitch,
  datadog,
  cosmonic,
  form3,
  kubesphere,
  meltwater,
  palantir,
  postfinance,
  scaleway,
  sportradar,
  utmost,
  yahoo,
  homeRetailer,
  overstock,
  ayedo,
  cognite,
  finleap,
  infomaniak,
  jumo,
  kubermatic,
  liquidReply,
  myFitnessPal,
  mux,
  theNewYorkTimes,
  nexxiot,
  northflank,
  radioFrance,
  rapyutaRobotics,
  sap,
  simple,
  kryptos,
  melenion,
  smileDirectClub,
  snapp,
  tsi,
  tailorBrands,
  f5,
  innoq,
  kubeOvn,
  mobilab,
  ect888,
  esnet,
  skybet,
  plaid,
  isovalent,
  schubergPhilis,
  spherity,
  frsca,
  rafay,
  hetzner,
  ascend,
  palark,
  incentive,
  jar,
  virtuozzo,
  alauda,
  netcloud,
  confluent,
  ebay,
  reddit,
  sony,
  qingcloud,
  imagineLearning,
  guidewire,
  sinad,
  docaposte,
  ecco,
  coreweave,
  kanabell,
  entrywan,
  streamSecurity,
  gResearch,
  cornerBanca,
  canopus,
  flipkart,
  guida,
  outscale,
  ovh,
  openai,
  preferredNetworks,
  batumbu,
  fptTelecom,
  ost,
  ionx,
  vnpt,
  airbnb,
  tiktok,
  bedrock,
  celonis,
  zynga,
  asana,
  etraveli,
  suse,
  telefonica,
  mogenius,
  cleverCloud,
  xata,
  pagali,
} = caseStudiesData;

const hero = {
  title: 'Adopters',
  description:
    'Here are some of the organizations we know are using Pixiu.<br/> If you’re using Pixiu and aren’t on this list, <a href="https://github.com/pixiu/pixiu/blob/master/USERS.md" target="_blank" rel="noopener noreferrer">please submit a pull request</a>!',
};

const userCommunity1 = {
  items: [
    FHNW,
    koyeb,
    bmc,
    syseleven,
    icos,
    kakao,
    samsungads,
    nemlig,
    gdata,
    smartnews,
    roche,
    schubergPhilis,
    sicredi,
    gcore,
    netbird,
    rabobank,
    santa,
    jsyk,
    vkcloud,
    superorbital,
    ilionx,
    viasat,
    giantSwarm,
    ws02,
    singlestore,
    deezer,
    parseable,
    dbraincloud,
    dbschenker,
    testifysec,
    palantir,
    edgelessSystems,
    trendyol,
    kubermatic,
    yahoo,
    eficode,
    clickhouse,
    tietoevry,
    bloomberg,
    azure,
    robinhood,
    vshn,
    adobe,
    theNewYorkTimes,
    seznam,
    sandp,
    postfinance,
    nexxiot,
    alibabaCloud,
    aws,
    sky,
    datadog,
    cosmonic,
    wildLife,
    capitalOne,
    masmovil,
    trip,
    cengn,
    digitalOcean,
    google,
    hetzner,
    bell,
    gitlab,
    uswitch,
    utmost,
    meltwater,
    homeRetailer,
    ect888,
    mobilab,
    skybet,
    form3,
    ascend,
    palark,
    jar,
    confluent,
    ebay,
    reddit,
    sony,
    qingcloud,
    imagineLearning,
    ecco,
    esnet,
    gResearch,
    cornerBanca,
    canopus,
    flipkart,
    bytedance,
    nutanix,
    cloudera,
    openai,
    cybozu,
    airbnb,
    tiktok,
    bedrock,
    uwmadison,
    ovh,
    zynga,
    asana,
    etraveli,
    suse,
    telefonica,
    cleverCloud,
  ],
};

userCommunity1.items.sort((a, b) =>
  a.iconName.toLowerCase().localeCompare(b.iconName.toLowerCase())
);

const userCommunity2 = {
  title: 'Pixiu is everywhere',
  isTitleCentered: true,
  items: [
    labyrinthLabs,
    scigility,
    cloutomate,
    qpoint,
    seatgeek,
    archer,
    VMwareTanzu,
    bitnami,
    linkpool,
    aenix,
    syself,
    spectrocloud,
    veepee,
    netways,
    sealos,
    sianalytics,
    plusserver,
    twilio,
    unitedcloud,
    frsca,
    rafay,
    innoq,
    f5,
    sproutfi,
    solo,
    overstock,
    accuknox,
    arangodb,
    acoss,
    canonical,
    celonis,
    melenion,
    civo,
    kubesphere,
    elasticpath,
    scaleway,
    sportradar,
    plaid,
    isovalent,
    spherity,
    ayedo,
    cognite,
    finleap,
    infomaniak,
    jumo,
    liquidReply,
    myFitnessPal,
    mux,
    northflank,
    radioFrance,
    rapyutaRobotics,
    sap,
    simple,
    smileDirectClub,
    snapp,
    tsi,
    tailorBrands,
    kubeOvn,
    kryptos,
    immerok,
    magicLeap,
    sapian,
    ungleich,
    planetscale,
    nine,
    kilo,
    polverio,
    polarsignals,
    daimlertruck,
    proton,
    cistec,
    kubehetzner,
    incentive,
    virtuozzo,
    alauda,
    netcloud,
    guidewire,
    sinad,
    docaposte,
    coreweave,
    kanabell,
    entrywan,
    streamSecurity,
    guida,
    outscale,
    ost,
    preferredNetworks,
    batumbu,
    fptTelecom,
    ionx,
    vnpt,
    mogenius,
    xata,
    pagali,
  ],
  buttonText: 'Add Your Company',
  buttonUrl: 'https://github.com/pixiu/pixiu/blob/master/USERS.md',
  buttonTarget: '_blank',
};

userCommunity2.items.sort((a, b) =>
  a.iconName.toLowerCase().localeCompare(b.iconName.toLowerCase())
);

const logos1 = {
  title: 'Deploy on your preferred cloud',
  items: [
    'googleCloud',
    'aws',
    'azure',
    'digitalOcean',
    'hetzner',
    'equinix',
    'openStack',
    'alibabaCloud',
    'scaleway',
    'sovereign',
    'syseleven',
    'civo',
    'nine',
    'tencentCloud',
    'daocloud',
    'exoscale',
    'appuio',
    'vkcloud',
    'netways',
    'sealos',
    'gcore',
    'qingcloud',
    'coreweave',
    'outscale',
    'ovh',
    'cleverCloud',
  ],
  spaceXSize: 'sm',
};

const logos2 = {
  title: 'Use your favorite Kubernetes distribution',
  items: [
    'azureKs',
    'openshift',
    'amazonEks',
    'constellation',
    'rke2',
    'googleKe',
    'gardener',
    'kind',
    'crane',
    'kops',
    'kubekey',
    'k8e',
    'kubeasz',
    'kubeone',
    'kubespray',
    'deckhouse',
    'alauda',
    'kubelift',
    'guida',
    'nutanix',
  ],
  spaceXSize: 'sm',
};

const Adopters = () => (
  <MainLayout className="overflow-x-hidden">
    <HeroWithoutImage {...hero} />
    <UserCommunity
      className="pt-6 pb-10 md:pt-10 md:pb-20 lg:pt-14 lg:pb-32 overflow-x-auto max-w-full"
      {...userCommunity1}
    />
    <Logos {...logos1} id="preferred-cloud" />
    <Logos {...logos2} id="kubernetes-distribution" />
    <UserCommunity
      className="mt-10 py-10 md:mt-20 md:py-20 lg:mt-28 lg:py-28 overflow-x-auto max-w-full"
      {...userCommunity2}
      id="pixiu-is-everywhere"
    />
  </MainLayout>
);

export default Adopters;

export const Head = ({ location: { pathname } }) => {
  const pageMetadata = { ...seo, slug: pathname };
  return <SEO data={pageMetadata} />;
};
