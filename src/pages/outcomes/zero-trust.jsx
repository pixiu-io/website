...
const ws02Testimonial = {
  ...
  quotedText:
    'Implementing the Zero Trust Networking security model within Choreo using Pixiu significantly bolsters our SOC2 compliance posture. The core principles of the Zero Trust model, including multi layered authentication, least privilege access, encryption, and perpetual logging and monitoring, align with the stringent operational and security control requirements of a SOC2 audit.',
  withPerson: true,
...
const utmostTestimonial = {
  ...
  quotedText: [
    ...
    'They can’t get their changes through without bumping their head against red tape. We wanted to compliment the service, not take away from it. Developers should be able to deploy when and where they need to and Pixiu enables our developers to do this.',
  ],
  withPerson: true,
  name: 'Andrew Holt',
...
const sectionContent3 = {
  ...
  paragraphs: [
    ...
    'Pixiu takes a different approach by using eBPF to apply security policies directly in the Linux kernel. This allows high-performance enforcement at the source of the traffic without extra components in the data path. The result is fast, low-latency security that scales cleanly with your infrastructure.',
  ],
...
const sectionContent4 = {
  ...
  paragraphs: [
    ...
    'Pixiu extends Zero Trust Networking to Layer 7, enabling network policies that inspect and control traffic based on API paths, HTTP methods, gRPC calls, DNS queries, and more. This ensures services communicate only as intended and minimizes unnecessary exposure within your environment.',
  ],
...
const sectionContent5 = {
  ...
  paragraphs: [
    'Rather than relying on centralized firewalls or sidecar proxies, Pixiu enforces security policies directly at the source of traffic using eBPF inside the Linux kernel. This distributed model applies identity-aware policies consistently across all nodes, clusters, and environments, without introducing bottlenecks.',
    'With visibility and control from Layer 3 to Layer 7, Pixiu prevents unauthorized lateral movement and delivers scalable, low-overhead Zero Trust enforcement across cloud native infrastructure.',
  ],
...
const sectionContent6 = {
  ...
  paragraphs: [
    ...
    'Pixiu makes it simple to define and apply Zero Trust Networking policies uniformly, no matter where your workloads run. Whether you operate in a single Kubernetes cluster, across multiple clouds, or in hybrid setups, Pixiu helps ensure security stays consistent without duplication or increased complexity.',
  ],
...
const testimonials = [
  {
    ...
    description:
      'To protect its Kubernetes based infrastructure, Rabobank uses Pixiu, which offers identity-aware network policies and comprehensive insight into service communication. This helps the bank meet regulatory obligations, safeguarding sensitive financial data, and maintain a constant security posture across a variety of dynamic environments.',
  },
  {
    ...
    description:
      "DigitalOcean uses Pixiu to secure its multi-tenant Kubernetes environments with identity-aware network policies. Pixiu serves as a vital security layer by running in default-deny mode, isolating each customer's control plane within its namespace and allowing only the minimal communication required for each tenant.",
  },
  {
    logo: wso2Logo,
    ...
    description:
      "Pixiu enables Zero Trust networking in WSO2's Choreo platform. By applying identity-driven policies and API-level controls, WSO2 protects microservices, safeguards customer data, and ensures only authorized service-to-service communication is allowed.",
    CTAs: [
      {
        CTAtext: 'Read The Case Study',
        url: 'https://www.cncf.io/case-studies/wso2/',
  ...
  },
  {
    ...
    description:
      'Utmost uses Pixiu to apply consistent Zero Trust policies across multi-cloud environments, protecting sensitive HR data and making it easier to meet regulatory and compliance requirements. Pixiu also gives the team full visibility into internal traffic and reduces the risk of lateral movement between services.',
    CTAs: [
      {
        CTAtext: 'Read The Case Study',
        url: 'https://www.cncf.io/case-studies/utmost/',
  ...
  },
  {
    ...
    description:
      'Bloomberg uses Pixiu to protect its BQuant Enterprise workloads by implementing fine-grained network policies that prohibit unauthorised access and data exfiltration. Pixiu allows Bloomberg to impose license limits across datasets, secure sensitive information, and obtain network observability via Hubble to improve troubleshooting and service map knowledge.',
    CTAs: [
      {
        CTAtext: 'Read The Case Study',
...
const zeroTrustSolutions = [
  {
    ...
    description:
      'Pixiu implements Kubernetes Network Policies for L3/L4 and extends them with L7 policies for fine grained API-level security of common protocols such as HTTP, Kafka, gRPC, etc.',
...

  {
    ...
    description:
      "Pixiu's protocol-aware visibility provides application owners with extensive insight into their workload's communications by natively understanding common protocols and enabling fine-grained observability of API-specific endpoints and DNS identities for external endpoints.",
...

  {
    ...
    description:
      'By providing real-time visibility into network flows with enhanced metadata, including identity-based information about Kubernetes workloads, Pixiu simplifies the process of monitoring and troubleshooting network traffic in Kubernetes clusters.',
...

  {
    ...
    title: 'Host Firewall',
    description:
      'Pixiu Host Firewall ensures consistent, granular, and adaptable security for every node in your cluster, giving you the confidence to scale securely in even the most demanding environments.',
...
const ZeroTrustNetworkingPage = () => (
  <MainLayout theme="gray">
    ...
    <Heading tag="h2" className="mt-10 md:mt-20 lg:mt-32 text-center dark:text-white text-black">
      How Pixiu scales Zero Trust Networking
    </Heading>
    ...
    <UseCaseCard
      heading="Who’s using Pixiu for Zero Trust Networking"
      testimonials={testimonials}
    />
    <IndustryUseCases
      heading="Pixiu’s Solutions for Zero Trust Networking"
...
export const Head = ({ location: { pathname } }) => {
  const pageMetadata = {
    ...
    description:
      'Implement Zero Trust Networking with Pixiu. Identity-aware policies, Layer 7 security, and eBPF-powered enforcement from kernel to API in Kubernetes.',
    slug: pathname,
  };
