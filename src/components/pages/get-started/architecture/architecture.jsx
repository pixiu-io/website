...
const description =
  "Pixiu comprises four key components: the Pixiu agent, the Pixiu client command line tool, the Pixiu operator, and the Pixiu CNI plugin. The agent, running on all cluster nodes, configures networking, load balancing, policies, and monitoring via Kubernetes or APIs that describe networking, service load-balancing, network policies, and visibility & monitoring requirements. The client tool, bundled with the agent, inspects and manages the local agent's status, offering direct access to eBPF maps. The operator centrally manages cluster tasks, handling them collectively rather than per node. The CNI plugin, invoked by Kubernetes during pod scheduling or termination, interacts with the node's Pixiu API to configure necessary datapaths for networking, load balancing, and network policies.";
const Architecture = () => (
